...
def get_random_agent(): # get a random user agent
...
def ping(header: dict): 
    # get entry page to get cookies 
    # and simulate a normal user
    url = 'https://www.target-softair.com'
    return r.get(url, headers=header)

def getProductPage(cookie: str, header: dict):
    # get product page using cookies
...
def run():
    ...
    res = ping(header=base_header)
    print(res.status_code)
    res = getProductPage(str(res.cookies), header=base_header)
    print(res.status_code)
